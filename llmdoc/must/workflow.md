...
| `liam-2.x` | 活跃开发分支，14 项定制功能基于 2.x | push/PR 触发构建，tag push 触发 Release |
...
权限：`contents: write`（创建 Release 所需）。

流程：
1. macos-latest，setup Xcode latest-stable
2. 安装 Procursus (ldid, findutils, sed, coreutils, make) + GNU Make (brew)
3. 恢复 THEOS 缓存（key=`theos-sdk16.5-${{ runner.os }}`），miss 时安装 THEOS + iPhoneOS16.5 SDK
4. 恢复 trustcache 缓存（key=`trustcache-${{ runner.os }}-${{ runner.arch }}`），miss 时编译 trustcache (OPENSSL=1)；始终将 binary 安装到 PATH
5. 设置环境变量：`COMPILE_TIME` = UTC+2 时间戳 `YYYYMMDD_HHMMSS`
6. 计算版本号（`version`）：tag 构建时直接用 `GITHUB_REF`，非 tag 时用 `git describe --tags`
7. 下载 Bootstraps (`Application/Dopamine/Resources/download_bootstraps.sh`)
8. `gmake -j NIGHTLY=1 DOPAMINE_VERSION="${version}"` 构建（注入 `MARKETING_VERSION`）
9. 上传 Artifact：`Dopamine.ipa` + `Dopamine.tipa`
10. 若为 tag push (`v*`)：创建 GitHub Release（自动生成 release notes）
...
- CI 版本号: tag 构建用 `GITHUB_REF`；非 tag 用 `git describe --tags`（如 `2.4.9-20260512_0044-16-g6d1f0372`）
- `DOPAMINE_VERSION` 非空时，`Application/Makefile` 将其传递为 xcodebuild 的 `MARKETING_VERSION`，覆盖 `CFBundleShortVersionString`

### Release 管理规约

- 当前仅保留一个非 prerelease（最新 2.x release）
- 历史正式 release 已全部转为 prerelease（69 个），历史 prerelease 已删除（7 个）
- 此策略使 app 内更新检查（`DOUIManager` 取最新非 prerelease）始终指向最新版本
